...
EFI_STATUS efi_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable){
	...
	while(running){
		...
		//Get number from the user until done
		while(1){
			...
			//Move the cursor left by 1, to overwrite the mode #
			...
			if (key.ScanCode == scancode_ESC){
				...
				//NOTE: This should not return, system should poweroff
				//__attribute__((unreac
			}
			
			//Choose text mode & readraw screen
